...
class Keyboard:
    def __init__(self, file_to_safe: str) -> None:
        self.buffer = Text() # для хранения текущего текста 
        # для сохранения/загрузки
        self.state_server = KeybordStateSaver(self, file_to_safe, KeyboardSerializer())
        # стек выполненых(отмена) команд 
        self.undo_stack: List[str] = []
        # стек отменненых(повтора) команд 
        self.redo_stack: List[str] = []
        # словарь доступных команд
        self.commands: Dict[str, Command] = {}
        # типы команд для восстановления
        self._command_types: Dict[str, Type[Command]] = {}

    def register_command(self, command_type: str, cls: Type[Command]) -> None:
        self._command_types[command_type] = cls # регестрируем тип команды для востановления

    def init_commands(self, commands: Dict[str, Command]) -> None:
        self.commands = commands # инициализация экзмпляра команд

    def do(self, command_key: str) -> None:
        if command_key not in self.commands: # проверка существует ли команда с таким ключом
            print(f"Command '{command_key}' not found")
            return
        cmd = self.commands[command_key]
        cmd.execute(self.buffer) # выполняет команду передавая ей буфер
        self.undo_stack.append(command_key)
        self.redo_stack.clear()

    def undo(self) -> None:
        if not self.undo_stack: # проверяет есть ли что отменять
            print("Nothing to undo")
            return
        command_key = self.undo_stack.pop()
        cmd = self.commands[command_key]
        cmd.cancel(self.buffer)
        self.redo_stack.append(command_key) # проверяет есть ли что отменять

    # выполняет команду снова и добавляет её обратно в стек отмены
    def redo(self) -> None:
...
@dataclass
class CommandData: # структура данных для сериализации команд
    type: str
    args: Dict[str, Any]
...
@dataclass
class KeyboardData:
    printed_sq: str # хранит текущее содержимое буфера
    undo_stack: List[str] # cписок ключей команд которые можно отменить
    redo_stack: List[str] # cписок ключей команд которые можно повторить
    commands: Dict[str, CommandData] = field(default_factory=dict) # cловарь всех доступных команд
    # field - при создании нового объекта KeyboardData будет создан новый пустой словарь
    # каждый экземпляр получит свою собственную копию словаря
...
class KeyboardSerializer(Serializer):
    def serialize(self, obj: object) -> Dict[str, Any]:
        ...
        # оставляет только те пары, ключи которых не входят в список исключений


class KeybordStateSaver:
    def __init__(self, keyboard: Keyboard, file_path: str,serializer: Serializer) -> None:
    ...
    def save(self) -> None: 
        commands_data = {
            key: CommandData(
                ...
                # type=cmd.__class__.__name__ — имя класса команды
                # args=self.serializer.serialize(cmd) — аргументы команды преобразованные в словарь
    ...
    def load(self) -> None:
        ...
        self.keyboard.buffer.set(data.get("printed_sq", "")) # берём значение по ключу "printed_sq"
                                                             # устанавливаем этот текст в буфер
        self.keyboard.undo_stack = data.get("undo_stack", []) # восстанавливаем стек отмены
        self.keyboard.redo_stack = data.get("redo_stack", []) # восстанавливаем стек повтора

        commands_data = data.get("commands", {})
        restored: Dict[str, Command] = {} # пустой словарь для восстановленных команд
...
commands: Dict[str, Command] = {letter: KeyCommand(letter) for letter in "abcd"}
